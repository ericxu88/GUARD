...
class Baseline:
    ...
    def validate(self) -> None:
        ...
        if self.class_distribution.shape != (c,):
            raise ValueError(f"class_distribution must be [{c}], got {tuple(self.class_distribution.shape)}")
        ...
        if self.embed_precision.shape != (d, d):
            raise ValueError(f"embed_precision must be [{d}, {d}], got {tuple(self.embed_precision.shape)}")
